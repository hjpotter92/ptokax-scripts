...
function Inflation()
	local sQuery = [[UPDATE `toks`
		SET `toks` = %f*`toks`]]
	local SQLCur = assert( sqlCon:execute(string.format(sQuery,tToksConfig.fInflationConstant)) )
end
function GrantAllowance()
	...
	for i,tMod in ipairs(tModerators) do
		assert( sqlCon:execute(string.format(sQuery2,tToksConfig.fModAllowance,tMod.sNick)))
	end
	local tVips = RegMan.GetRegsByProfile(3)
	for i,tVip in ipairs(tVips) do
		assert( sqlCon:execute(string.format(sQuery2,tToksConfig.fVipAllowance,tVip.sNick)))
	end
	local tOperators = RegMan.GetOps()
	for i,tOp in ipairs(tOperators) do
		assert( sqlCon:execute(string.format(sQuery2,tToksConfig.fOpAllowance,tOp.sNick)))
	end
end
...
local function GetAttribute (sNick ,sAttribute)
	...
	tSQLResults = assert( sqlCon:execute(string.format(sQuery,sAttribute,sNick)) )
...
function gift(sDonorNick,sDoneeNick,fAmount,sMessage)
	...
	local sQuery1 =  "UPDATE `toks` SET `allowance` = `allowance` - %f WHERE username = '%s'"
	Core.SendPmToNick("Brick","PtokaX",string.format(sQuery1,fAmount,sDonorNick))
	assert( sqlCon:execute(string.format(sQuery1,fAmount,sDonorNick)))
	local sQuery2 =  "UPDATE `toks` SET `toks` = `toks` + %f WHERE username = '%s'"
	assert( sqlCon:execute(string.format(sQuery2,fAmount,sDoneeNick)))
	local sQuery3 = "INSERT INTO `transactions`(`from`,`to`,`amount`,`date`) VALUES ('%s','%s',%.3f,CURRENT_TIMESTAMP)"
	assert( sqlCon:execute(string.format(sQuery3,sDonorNick,sDoneeNick,fAmount)))
